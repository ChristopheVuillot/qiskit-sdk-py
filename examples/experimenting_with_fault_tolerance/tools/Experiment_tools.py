...
def convert_parameter(parameter):
    if 'unit' in parameter:
        if parameter['unit'] == 'ns':
            multiplier_p = 10**(-9)
        elif parameter['unit'] == 'µs':
            multiplier_p = 10**(-6)
        elif parameter['unit'] == 'ms':
            multiplier_p = 10**(-3)
        else:
            multiplier_p = 1
    else:
        multiplier_p = 1
    return parameter['value']*multiplier_p

def convert_rate(time_t, time_gate):
    return 1-np.exp(-convert_parameter(time_gate)/convert_parameter(time_t))
